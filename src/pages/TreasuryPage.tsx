...
export const TreasuryPage: React.FC<{ profile: Profile }> = ({ profile }) => {
  ...
  const openMovementModal = (type: 'in' | 'out' | 'transfer') => {
    setModalType(type);
    setErrorMsg('');
    ...
    <div className="space-y-5" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex w-full items-center justify-between gap-3 sm:w-auto sm:justify-end">
          <div className="hidden items-center gap-1.5 rounded-2xl border border-slate-200 bg-slate-50 p-1 sm:flex">
            <button
            ...
            <button
              onClick={() => openMovementModal('transfer')}
          ...
          <div className="flex items-center gap-2 sm:mr-2 sm:border-r sm:border-slate-200 sm:pr-2">
            <div className="relative sm:hidden">
              ...
              {isActionsOpen && (
                <div className="absolute left-0 top-full z-20 mt-2 w-52 overflow-hidden rounded-xl border border-slate-200 bg-white p-1 shadow-xl" role="menu">
                  ...
                  <button type="button" role="menuitem" onClick={() => openMovementModal('out')} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-right text-xs font-bold text-rose-700 transition hover:bg-rose-50">
                    <Minus className="h-4 w-4" /> مصروف / نقدية
                  </button>
                  <button type="button" role="menuitem" onClick={() => openMovementModal('transfer')} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-right text-xs font-bold text-slate-800 transition hover:bg-slate-50">
                    <Send className="h-4 w-4" /> تحويل لمحفظة / ماكينة
                  </button>
                </div>
              )}
            </div>
